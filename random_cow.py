    ...
    def set_cow_dir(self):
        ...
        try:
            ...
            error_message = "".join(["ASCII files containing cows could not be found because the default directory,",
                                    ...
                                    "if you are on a Debian based system, use the Synaptic GUI application to find the ",
        ...
        for key in ["COWDIR", 'cowdir', 'cow_dir']:
            try:
                ...
                if (len(self.cow_dir) is 0) or (not isinstance(env[key], str)): raise KeyError
            except KeyError:
                syslog.syslog(syslog.LOG_ERR, "{env_var_name} environment variable is unset or empty.".format(env_var_name = key))
                raise SystemExit("Unable to find the cow files. Quitting.")

    def get_cow_file(self, cow_dir):
